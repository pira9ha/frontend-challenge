...
import { CustomSvg, Heart } from '../styles/styledComponenets';
import { deleteCatAsFavorite, saveCatAsFavorite } from '../../service/axiosWorker';
import { CardProps } from '../../models/propsTypes';
import { removeFromStorage, setItemInStorage } from '../../service/localStorage.service';
...
export const LikeButton = ({ item }: CardProps) => {
  ...
  useEffect(() => {
    if (state && state.favorites) {
      const existFavorite = state.favorites.find((cat) => cat.id === item?.id);
      if (existFavorite)
        setLiked(true);
    }
  }, [state?.favorites]);
  ...
  return(
    <Heart onClick={() => {
      if (!liked) {
        saveCatAsFavorite(item).then(() => setItemInStorage(item));
        setLiked(true);
        state?.favorites.push(item);
      } else {
        deleteCatAsFavorite(item).then(() => removeFromStorage(item));
      }
    }}>
      <CustomSvg width="40" height="37" viewBox="0 0 40 37" fill="none" xmlns="http://www.w3.org/2000/svg" className="active">
      ...
      <CustomSvg width="40" height="37" viewBox="0 0 40 37" fill="none" xmlns="http://www.w3.org/2000/svg" className={`inactive${ liked ? ' liked' : ''}`}>
